...
$(document).on("click", "#receive_package", receivePackage);
//Functions
...
function checkInputs(e) {
    ...
    if (checkMtInps(inps_required, 'pink') == false) {
        ...
        let mt_date_received_date_time = moment(date_received.value + ' ' + time_received.value);
        let mt_received_date_time = moment(received_date.value + ' ' + received_time.value);
        let diff_day_time = mt_received_date_time.diff(mt_date_received_date_time);
        if (diff_day_time <= 0) {
            e.preventDefault();
            bootbox.alert("<p class='alert alert-danger text-center font-weight-bold'>Check Receiving and Sending Date & Time!</p>");
        }
    }
}

function receivePackage(e) {
    e.preventDefault();
    let inps_required = ["received_date", "received_time", "received_package_description", "received_package_qty", "received_collection_date", "received_collection_time"];
    if (checkMtInps(inps_required, 'pink')) {
        let url = '/mzigohuoo/SalesCounter/receivePackageAjax';
        let form_data = new FormData();
        let post_ids = ["received_date", "received_time", "received_package_description", "received_package_qty", "received_collection_date", "received_collection_time",
            "sending_package_id", "csrf_token"
        ];
        post_ids.forEach(function(post_id) {
            form_data.append(post_id, document.getElementById(post_id).value);
        });
        let success_action = function(fb_data) {
            ...
            sending_package_status.value = "ARRIVED";
        };
